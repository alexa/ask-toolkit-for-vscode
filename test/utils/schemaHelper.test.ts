...
describe("SkillPackageSchema tests", () => {
  ...
  describe("getSchemaLocationUrl function tests", () => {
    ...
    it("should throw AskError when failed to get resource schema", async () => {
      ...
      try {
        ...
        assert.strictEqual(e.name, "AskError");
        assert.strictEqual(e.message, `Failed to get skill-package schema for vendorId ${FAKE_VENDOR_ID}. Reason: testError`);
      }
    });
  ...
  describe("updateSchemas", () => {
    ...
    it("when schema folder not exist, should create a new schema folder", async () => {
      ...
      const updateSkillPackageSchemaSpy = sandbox.stub(SchemaManager.getInstance() as any, "updateSkillPackageSchema").resolves();
      const registerSkillPackageWatcherSpy = sandbox.stub(SchemaManager.getInstance() as any, "registerSkillPackageWatcher");
