...
describe('Keyword Patterns', function () {
  ...
  it('should match other keyword pattern', function () {
    assert.deepStrictEqual(['keyword.other.acdl',
      '\\b(dialog|sample|type|action|import|namespace)\\b'], Object.values(keywords.patterns[3]));
  });
});
...
describe('String Patterns', function () {
  ...
  it('should return correct string pattern', function () {
    assert.deepStrictEqual([['name', 'string.quoted.double.acdl'],
    ['begin', '"'],
    ['end', '"']], Object.entries(stringMatch));
  });
});
...
describe('Comment Patterns', function () {
  ...
  it('should match comment line pattern', function () {
    assert.deepStrictEqual([['name', 'comment.line.acdl'], ['match', '//.*$']], Object.entries(comment.patterns[0]));
  });
  it('should match comment block pattern', function () {
    assert.deepStrictEqual([['name', 'comment.block.acdl'],
    ['begin', '/\\*'],
    ['end', '\\*/']], Object.entries(comment.patterns[1]));
  });
});
...
describe('Types Patterns', function () {
  ...
  it('should match types begin pattern', function () {
    assert.deepStrictEqual('(type){1}\\s+(([A-Za-z_0-9\\-]+[\\.]{1})*)([A-Za-z_0-9\\-]+)\\s*([:]{1}\\s*(([A-Za-z_0-9\\-]+[\\.]{1}))*([A-Za-z_0-9\\-]+))?\\s*({)', types.begin);
    assert.deepStrictEqual({
      '1': { name: 'keyword.other.acdl' },
