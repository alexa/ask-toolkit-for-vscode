...
describe("SkillPackageWatcher", () => {
  ...
  describe("constructor tests", () => {
    ...
    it("when unable to get skillPackagePath, should not register the watcher", () => {
      ...
      assert.ok(
        warningMessageSpy.calledOnceWith(`Failed to read skill package path due to ${fakeError}, the skill package watcher won't start.`),
      );
  ...
  describe("validate tests", () => {
    ...
    beforeEach(() => {
      getSkillMetadataSrcStub = sandbox.stub(skillHelper, "getSkillMetadataSrc").returns({skillPackageAbsPath: validSkillPackagePath});
    ...
    it("validate function should restart the watcher when skillPackagePath changed", () => {
      ...
      sandbox.stub(skillHelper, "getSkillMetadataSrc").returns({skillPackageAbsPath: path.join(validSkillPath, "skill-package-NHS")});
    ...
    it("should display warning message when skill.json not exist", () => {
      ...
      assert.ok(disposeSpy.calledOnce);
      assert.ok(warningMessageSpy.calledWith(`skill.json is missing in skill-package. ${SKILL_PACKAGE_FORMAT_GUID}`));
    });
