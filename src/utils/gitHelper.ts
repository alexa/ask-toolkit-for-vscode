...
export class GitInTerminalHelper {
    ...
    configureCredentialHelper(repoUrl: string, profile: string, skillId: string): void {
        const credentialHelperPath = path.join(
            homedir(), SYSTEM_ASK_FOLDER.HIDDEN_ASK_FOLDER, SYSTEM_ASK_FOLDER.SCRIPTS_FOLDER.NAME,
    ...
    static addFilesToIgnore(targetPath: string, fileNames: Array<string>): void {
        Logger.verbose(`Calling method: addFilesToIgnore, args: `, targetPath, fileNames);
        const gitignorePath = path.join(targetPath, '.gitignore');
        if (!fs.existsSync(gitignorePath)) {
            fs.writeFileSync(
                gitignorePath, `${fileNames.join('\n')}`, 
                {
                    mode: 0o600
                }
            );
    ...
    private _execCommands(commands: Array<string>, options: any): void {
