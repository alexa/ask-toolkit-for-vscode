...
export async function checkAskPrePushScript(context: vscode.ExtensionContext): Promise<void> {
  ...
  const askFilePath = path.join(
    ...
    SYSTEM_ASK_FOLDER.SCRIPTS_FOLDER.ASK_PRE_PUSH,
  );
...
async function isScriptUpdated(scriptUrl: string, scriptPath: string): Promise<boolean> {
...
async function downloadScriptFromS3(scriptUrl: string, filePath: string, chmod: string | undefined): Promise<void> {
