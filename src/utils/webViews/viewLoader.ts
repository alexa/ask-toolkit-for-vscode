  ...
  public renderView(options: RenderOptions): string {
    ...
    if (options.images) {
      for (const image of options.images) {
        const imageLocation: Uri | undefined = webview.asWebviewUri(
          Uri.file(path.join(this.context.extensionPath, "media", this.subdir, image + ".png")),
        );
        if (!imageLocation) {
          return this.handleError(options.errorMsg);
        }
    ...
    if (options.js) {
      const jsLocation: Uri | undefined = webview.asWebviewUri(
        Uri.file(path.join(this.context.extensionPath, "media", this.subdir, options.name + ".js")),
      );
      if (!jsLocation) {
        return this.handleError(options.errorMsg);
      }
    ...
    if (options.customCss) {
      ...
      const toolkitCss: Uri | undefined = webview.asWebviewUri(Uri.file(path.join(this.context.extensionPath, "media", "toolkit.css")));
      if (!toolkitCss) {
        return this.handleError(options.errorMsg);
      }
