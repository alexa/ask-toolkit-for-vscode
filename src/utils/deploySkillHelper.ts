...
async function checkValidStage(
  branchName: string,
  skillId: string,
  smapiClient: model.services.skillManagement.SkillManagementServiceClient,
): Promise<void> {
...
async function checkInProgressCerts(
  ...
  if (certificationListResponse.items?.some((certSummary) => certSummary.status === "IN_PROGRESS")) {
    throw logAskError(
...
async function pollSkillBuildStatus(
  ...
  commitId: string,
  smapiClient: model.services.skillManagement.SkillManagementServiceClient,
): Promise<SkillBuildResponseType | undefined> {
  ...
  return await retry(async (bail: (err: Error) => void, attempt: number) => {
    ...
    response.manifest.status = view(lensPath([SKILL.RESOURCES.MANIFEST, "lastUpdateRequest", "status"]), skillStatus);
    response.manifest.errors = view(lensPath([SKILL.RESOURCES.MANIFEST, "lastUpdateRequest", "errors"]), skillStatus);
    ...
    if (skillStatus[SKILL.RESOURCES.INTERACTION_MODEL]) {
      ...
      buildDetails = view(lensPath([SKILL.RESOURCES.INTERACTION_MODEL, locale, "lastUpdateRequest", "buildDetails"]), skillStatus);
    }
...
export async function deploySkill(skillFolderWs: vscode.Uri, context: vscode.ExtensionContext, view: AbstractWebView): Promise<void> {
  ...
  try {
    ...
    vscode.window.withProgress(
      ...
      async (progress, token) => {
        ...
        try {
          await pollSkillBuildStatus(skillId, latestCommit.hash, smapiClient);
