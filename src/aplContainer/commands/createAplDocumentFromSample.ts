...
export class CreateAplDocumentFromSampleCommand extends AbstractCommand<void> {
  ...
  public async execute(context: CommandContext, skillFolderWs: vscode.Uri): Promise<void> {
    ...
    const pickedSampleItem: SampleTemplateQuickPickItem | undefined = await vscode.window.showQuickPick(this.getSampleOptions(), {
   ...
   */
  private async createAplandDataFileFromSample(pickedSampleItem: SampleTemplateQuickPickItem, resourceDirPath: string): Promise<void> {
    const pickedSampleTemplate: ISampleTemplate | undefined = this.sampleTemplates.get(pickedSampleItem.id);
    if (!pickedSampleTemplate) {
      throw logAskError(
        ERROR_MESSAGES.CREATE_APL_FROM_SAMPLE_NO_SAMPLE_TEMPLATE_FOUND,
        new Error(`No sample template found for ${pickedSampleItem.label}`),
        true,
      );
    }
