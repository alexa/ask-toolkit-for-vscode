...
export class PreviewAplCommand extends AbstractCommand<void> {
   ...
   * @memberOf PreviewAplCommand
   */
  private async openAplResourceInTextEditor(
    aplResourceRootPath: string,
    pickAplResourceIdentifier: string,
    ...
    if (fs.existsSync(datasourcesPath)) {
      await vscode.window.showTextDocument(vscode.Uri.file(datasourcesPath), {
        preview: false,
        viewColumn,
    ...
    if (fs.existsSync(aplDocumentPath)) {
      await vscode.window.showTextDocument(vscode.Uri.file(aplDocumentPath), {
        preview: false,
        viewColumn,
      } as vscode.TextDocumentShowOptions);
    } else {
