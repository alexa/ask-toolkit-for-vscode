...
export class AplPreviewWebView extends AbstractWebView {
  ...
  constructor(viewTitle: string, viewId: string, context: vscode.ExtensionContext) {
    ...
    super(viewTitle, viewId, context, vscode.ViewColumn.Beside, allowedLocalResourceUri, {
      preserveFocus: true,
  ...
  update(document: vscode.TextDocument): void {
    ...
    if (DOCUMENT_PATH_REGEX.test(textDocumentPath)) {
      ...
      const datasourcesPath: string = path.join(path.dirname(textDocumentPath), DATASOURCES_FILE_PATH);
      const datasources: string = fs.existsSync(datasourcesPath) ? this.getJson(datasourcesPath) : "{}";
      this.document = document;
      this.datasources = datasources;
    } else if (DATA_PATH_REGEX.test(textDocumentPath)) {
      const documentPath: string = path.join(path.dirname(path.dirname(textDocumentPath)), APL_DOCUMENT_FILE_PATH);
  ...
  getHtmlForView(...args: any[]): string {
    ...
    const customJavascript = this.getWebview()
