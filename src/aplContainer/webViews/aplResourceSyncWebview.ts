...
import {
  ...
  DATASOURCES_PATH_REGEX_RELATIVE_TO_SKILL_PACKAGE,
  DOCUMENT_PATH_REGEX_RELATIVE_TO_SKILL_PACKAGE,
  SOURCES_FILE_PATH,
  SOURCES_PATH_REGEX_RELATIVE_TO_SKILL_PACKAGE,
...
export class AplResourceSyncWebview extends AbstractWebView {
  ...
  async retrieveAplResources(): Promise<void> {
    ...
    await vscode.window.withProgress(
      ...
      async (_progress) => {
        ...
        const skillPkgZipLocation: string = await getSkillPkgZipLocation(this.wsFolder.fsPath, skillId, this.extensionContext);
  ...
  async updateAplResource(name: string): Promise<void> {
    ...
    if (this.aplResourceMap && this.aplResourceMap.has(name)) {
      ...
      if (sources) {
        const sourcesPath: string = path.join(
          this.wsFolder.fsPath,
          displayDirRootPath(this.wsFolder.fsPath, profile),
          name,
          SOURCES_FILE_PATH,
        );
      ...
      if (datasources) {
        const dataSourcePath: string = path.join(
          this.wsFolder.fsPath,
          displayDirRootPath(this.wsFolder.fsPath, profile),
          name,
          DATASOURCES_FILE_PATH,
