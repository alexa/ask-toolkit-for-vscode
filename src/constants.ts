...
export const SKILL_ACTION_URLS = {
  IMODEL_EDITOR: (skillId: string, locale: string): string =>
    `${DEV_CONSOLE_ENDPOINT}/build/custom/${skillId}/development/${locale}/intents`,
...
export const MASTER_BRANCH_SKILL_MODELS_URL = "https://raw.githubusercontent.com/alexa/skill-sample-nodejs-hello-world/master/models";
export const MASTER_BRANCH_SKILL_MANIFEST_URL = "https://raw.githubusercontent.com/alexa/skill-sample-nodejs-hello-world/master/skill.json";
...
export const ERRORS = {
  SKILL_ACCESS: (currentProfile: string, skillProfile: string): string =>
    `'${currentProfile}' profile doesn't have access to the skill. Please use '${skillProfile}' profile.`,
  ...
  UNRECOGNIZED_ALEXA_RESPONSE_TYPE: (responseType: string | undefined): string =>
    `The simulator returned an unrecognized Alexa response of type '${responseType}.'`,
  UNDEFINED_SIMULATION_ID: "Error: The simulator ID was invalid. Try again.",
  UNRECOGNIZED_SIMULATION_RETURN_MESSAGE:
    "Error: Unrecognized simulation return message. Message must contain either 'error' or 'alexaResponse'.",
  ...
  OPEN_REPLAY_FILE_FAIL: (filePath: string | undefined): string =>
    `Open replay file failed. File path: '${filePath}'. Please check the json file.`,
  ...
  REPLAY_FAILED_ENABLE:
    'Failed to import replay file because the Simulator is off. Turn the Skill stage to "development" before importing a replay file.',
  EXPORT_FAILED: "There is no utterance to export, please input some utterances.",
  EXPORT_FILE_FAIL: (filePath: string | undefined): string =>
    `The file '${filePath}' could not be saved. Please make sure you have access to write to this path.`,
  PROFILE_ERROR: (profile: string | undefined): string =>
    `This skill cannot be accessed by the current profile: ${profile}. To access this skill, switch to a different profile.`,
  ...
  CREDENTIAL_EXPIRED: (deviceId: string): string =>
    `The device credentials for ${deviceId} is expired. Do you want to reauthorize the device?`,
};
...
export const SKILL_NOT_DEPLOYED_MSG =
  "This skill has not been previously deployed. Some extension functionalities will be unavailable until an initial deployment.";
export const MULTIPLE_SKILLS_MSG =
  "Multiple skills are open in the workspace, so some Alexa Toolkit Extension features will not work. Alexa Toolkit Extension works best when only one skill is open.";
export const MULTIPLE_SKILLS_PRESENT_MSG =
  "The workspace already contains one or more skill folders. Alexa Toolkit Extension works best when only one skill is open. Do you want to open the skill in a new workspace?";
export const CLI_V1_GLOB_PATTERN = "**/.ask/config";
export const CLI_V1_SKILL_MSG =
  "This skill contains unsupported skill folder structure. Some Alexa Toolkit Extension features will not work. Please follow [this guide](https://developer.amazon.com/en-US/docs/alexa/smapi/ask-cli-v1-to-v2-migration-guide.html#new-skill-project-structure-required-for-deployment-with-cli-v2) to upgrade your skill folder structure.";
...
export const SKILL_PACKAGE_FORMAT_GUID =
  "Please follow [this guide](https://developer.amazon.com/en-US/docs/alexa/smapi/skill-package-api-reference.html#skill-package-format) to upgrade your skill-package folder structure. For more detailed skill-package format, please check .ask/schema/skillPackageSchema.json.";
...
export const DEPLOY_HOSTED_LOCAL_CHANGE_STATE_CONTENT = Object.freeze({
  UNTRACKED: (branch: string) =>
    `Untracked file(s) present in the current branch <code>${branch}</code>. Use <code>git add</code> to track.`,
  UNSTAGED: (branch: string) =>
    `Changes exist in the current branch <code>${branch}</code>. Use <code>git add</code> and <code>git commit</code> to commit changes.`,
  NO_CHANGE: (branch: string) =>
    `There are no changes in the current branch <code>${branch}</code> to deploy. To deploy this skill, commit a change to the project.`,
  INVALID_BRANCH:
    "The current branch must be either <code>master</code> or <code>prod</code>.</p> Merge changes to these branches to deploy the skill.",
