...
export function listExistingProfileNames(): string[] | null {
  const askConfig: string = join(homedir(), FILE_PATH.ASK.HIDDEN_FOLDER, FILE_PATH.ASK.PROFILE_FILE);
...
export function getProfileInfo(profile: string): Hash | null {
  const askConfig = join(homedir(), FILE_PATH.ASK.HIDDEN_FOLDER, FILE_PATH.ASK.PROFILE_FILE);
  if (!existsSync(askConfig)) {
    return null;
  }
...
export async function setVendorId(profile: string, context: vscode.ExtensionContext): Promise<boolean> {
  ...
  const configPath = join(homedir(), FILE_PATH.ASK.HIDDEN_FOLDER, FILE_PATH.ASK.PROFILE_FILE);
