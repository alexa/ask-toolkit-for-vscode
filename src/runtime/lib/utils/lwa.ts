...
export async function accessTokenGenerator(lwaOptions: any): Promise<Token> {
  const {clientId, clientSecret, lwaAuthorizeHost, lwaTokenHost, scopes = DEFAULT_SCOPES, state = DEFAULT_STATE} = lwaOptions;
  ...
  return new Promise<string>((resolve) => {
    if (request.url) {
      ...
      if (request.url.startsWith("/cb?code")) {
        ...
        throw new Error(
          "Access not granted. Please verify your account credentials are correct.\nIf this is your first time getting the error, " +
