...
export class ListSkillsCommand extends AbstractCommand<Array<SmapiResource<skillSummary>>> {
  ...
  private async getSkillsList(
    smapiClientInstance: skillManagementServiceClient,
    vendorId: string,
    nextToken: undefined | string,
    skillsList: Array<SmapiResource<skillSummary>>,
  ): Promise<Array<SmapiResource<skillSummary>>> {
    const listSkills: model.v1.skill.ListSkillResponse = await smapiClientInstance.listSkillsForVendorV1(vendorId, nextToken);
  ...
  async execute(context: CommandContext): Promise<Array<SmapiResource<skillSummary>>> {
    ...
    await this.getSkillsList(SmapiClientFactory.getInstance(profile, context.extensionContext), vendorId, undefined, skills);
    return skills;
  }
}
