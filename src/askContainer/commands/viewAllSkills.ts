...
import {clearCachedSkills, getCachedSkills, getSkillMetadata, getSkillNameFromLocales, setCachedSkills} from "../../utils/skillHelper";
...
export class ViewAllSkillsCommand extends AbstractCommand<void> {
    ...
    await Promise.allSettled(
      this.skillsList.map(async (skill, index) => {
        ...
        try {
          ...
          skillMetadata = await retry(async (bail: (err: Error) => void): Promise<hostedSkillMetadata | undefined> => {
            try {
              return await smapiClient.getAlexaHostedSkillMetadataV1(skill.data.skillSummary.skillId ?? "");
            } catch (err) {
              if (err.name === "ServiceError" && err["statusCode"] === 429) {
                throw new AskError(`API throttled. Retrying after few seconds`);
              }
  ...
  private async setSkillsList(): Promise<void> {
    ...
    let skillsArray: Array<SmapiResource<skillSummary>> | undefined = await vscode.commands.executeCommand("ask.container.listSkills");
    ...
    if (skillsArray) {
      ...
      skillsArray.forEach((skill) => {
        this.skillsList.push(new SmapiResource<SkillInfo>(new SkillInfo(skill.data, undefined), skill.data.skillId ?? ""));
      });
    }
  }

  private async setQpItems(context: vscode.ExtensionContext, qp: vscode.QuickPick<vscode.QuickPickItem>): Promise<void> {
  ...
  private async checkAndSetQpItems(context: vscode.ExtensionContext, qp: vscode.QuickPick<vscode.QuickPickItem>): Promise<void> {
      ...
      if (allSkillsQP.activeItems.length !== 0) {
        ...
        getSkillMetadata(skillInfo.data.skillSummary.skillId!, SKILL.STAGE.DEVELOPMENT, context.extensionContext)
          .then(() => {
            ...
            void vscode.commands.executeCommand(executeCommand, skillInfo);
          })
