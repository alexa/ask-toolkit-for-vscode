...
export class SkillPackageWatcher {
  ...
  private getSkillPackageSchema(): SkillPackageSchema | undefined {
    ...
    try {
      ...
      const warningMessage = `Failed to read skill package schema due to ${err}, the skill package watcher won't start.`;
  ...
  private getSkillPackagePath(): string | undefined {
    try {
      ...
      const warningMessage = `Failed to read skill package path due to ${err}, the skill package watcher won't start.`;
      void vscode.window.showWarningMessage(warningMessage);
      Logger.debug(warningMessage);
