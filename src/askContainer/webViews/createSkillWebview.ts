...
export class CreateSkillWebview extends AbstractWebView {
  ...
  async onReceiveMessageListener(message: string | createSkillWebViewType): Promise<void> {
    ...
    if (message === "selectFolder") {
      ...
      if (
        !Utils.isNonBlankString(message.skillName) ||
        !Utils.isNonBlankString(message.skillFolder) ||
        !Utils.filterNonAlphanumeric(message.skillName)
      ) {
  ...
  private createSkillFolder(skillFolder: string, skillName: string): vscode.Uri {
    ...
    if (fs.existsSync(skillFolderAbsPath)) {
      vscode.window.showWarningMessage(`Skill folder ${skillFolderAbsPath} already exists. It will be overwritten.`);
  ...
  private async createSmapiResource(skillId: string, skillName: string): Promise<SmapiResource<SkillInfo>> {
    ...
    const skillSummary = (await smapiClient.listSkillsForVendorV1(vendorId, undefined, undefined, [skillId])).skills?.[0];
    if (!skillSummary) {
  ...
  private async createSkillFlow(
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        await cloneSkill(smapiResource, skillFolderUri.fsPath, this.extensionContext, progress);
  ...
  private async authenticateNewUser(): Promise<void> {
    ...
    profile = profile ?? DEFAULT_PROFILE;
    const vendorId = Utils.resolveVendorId(profile);
    const permission = await SmapiClientFactory.getInstance(profile, this.extensionContext).getAlexaHostedSkillUserPermissionsV1(
      vendorId,
      "newSkill",
    );
