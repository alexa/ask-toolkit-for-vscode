...
export class CreateHostedSkillManager extends AbstractCreateSkillManager {
  ...
  async requestCreateSkill(userInput: createSkillWebViewType, progress: Progress<any>): Promise<string> {
    ...
    const createSkillResponse = await SmapiClientFactory.getInstance(this.profile, this.context).createSkillForVendorV1(payload);
  ...
  private async pollSkillCreation(skillId: string): Promise<any> {
    ...
    await retry(async (bail: (err: Error) => void, attempt: number) => {
      ...
      const skillStatus: any = await SmapiClientFactory.getInstance(this.profile, this.context).getSkillStatusV1(skillId);
      let buildDetails;
  ...
  private async generateSmapiResource(skillId: string, skillName: string) {
    ...
    const skillSummary = (await smapiClient.listSkillsForVendorV1(this.vendorId, undefined, undefined, [skillId])).skills?.[0];
    if (!skillSummary) {
