...
export class CreateSkillWebview extends AbstractWebView {
  ...
  private createSkillFolder(skillFolder: string, skillName: string): vscode.Uri {
    ...
    if (fs.existsSync(skillFolderAbsPath)) {
      void vscode.window.showWarningMessage(`Skill folder ${skillFolderAbsPath} already exists. It will be overwritten.`);
  ...
  private async createSkillFlow(
    ...
    Logger.verbose(`Calling method: ${this.viewId}.createNonHostedSkillFlow, args: `, profile, message, skillFolderUri, isHostedSkill);
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        if (isHostedSkill === false) {
          const createNonHostedSkillManager = new CreateNonHostedSkillManager(this.context, skillFolderUri.fsPath);
