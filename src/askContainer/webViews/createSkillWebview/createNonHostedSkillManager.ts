...
export class CreateNonHostedSkillManager extends AbstractCreateSkillManager {
  ...
  private cloneSkillTemplate(userInput: createSkillWebViewType) {
    ...
    const branch = userInput.language === "Java" ? TEMPLATES.TEMPLATE_ASK_TOOLS_BRANCH : TEMPLATES.TEMPLATE_ASK_CLI_BRANCH;
  ...
  private async updateSkillTemplate(skillName: string, locale: string): Promise<void> {
    ...
    // remove assets
    const assestsFolder = path.join(this.skillFolder, SKILL_FOLDER.SKILL_PACKAGE.NAME, SKILL_FOLDER.SKILL_PACKAGE.ASSETS);
  ...
  private async updateSkillManifest(locale: string, skillName: string) {
    ...
    const manifestFilePath = path.join(this.skillFolder, SKILL_FOLDER.SKILL_PACKAGE.NAME, SKILL_FOLDER.SKILL_PACKAGE.MANIFEST);
  ...
  private async updateInteractionModels(locale: string) {
    ...
    const newIM = path.join(this.skillFolder, SKILL_FOLDER.SKILL_PACKAGE.NAME, SKILL_FOLDER.SKILL_PACKAGE.CUSTOM_MODELS, locale + ".json");
