...
export class SimulateSkillWebview extends AbstractWebView {
  ...
  getHtmlForView(): string {
    ...
    simulateCss = this.getWebview().asWebviewUri(vscode.Uri.file(path.join(this.extensionContext.extensionPath, "media", "simulate.css")));
    aplRenderUtils = this.getWebview().asWebviewUri(
      vscode.Uri.file(path.join(this.extensionContext.extensionPath, "media/previewApl", "aplRenderUtils.js")),
    );
    customJavascript = this.getWebview()
      .asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, "/node_modules/apl-viewhost-web/index.js")))
      .toString();
  ...
  async onReceiveMessageListener(message: Record<string, any>): Promise<void> {
    ...
    switch (message.type) {
      case SIMULATOR_MESSAGE_TYPE.SKILL_STATUS: {
        const returnMessage: string | Record<string, any> = await simulateMessageHelper.handleSkillStatusMessageFromWebview(
          message,
          profile,
          skillId,
      ...
      case SIMULATOR_MESSAGE_TYPE.LOCALE: {
        const returnMessage: void | Record<string, any> = await simulateMessageHelper.handleLocaleMessageFromWebview(
          message,
          profile,
          skillId,
          this.extensionContext,
      ...
      case SIMULATOR_MESSAGE_TYPE.UTTERANCE: {
        const returnMessage = await simulateMessageHelper.handleUtteranceMessageFromWebview(
          message,
          profile,
          skillId,
          this.extensionContext,
  ...
  private async reauthorizeDevice() {
    ...
    if (registeredDeviceId && registeredDeviceId !== null) {
      ...
      const option = await vscode.window.showErrorMessage(ERRORS.CREDENTIAL_EXPIRED(registeredDeviceId), {modal: true}, confirmAuth);
      ...
      if (option === confirmAuth) {
        ...
        this.isAVSMode = false;
        await this.extensionContext.globalState.update(EXTENSION_STATE_KEY.REGISTERED_DEVICE.VALID_DEVICE, "false");
