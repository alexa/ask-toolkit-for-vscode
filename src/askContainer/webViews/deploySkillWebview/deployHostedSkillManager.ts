  ...
  async pollSkillBuildStatus(commitId: string): Promise<SkillBuildResponseType | undefined> {
    ...
    return retry(async (bail: (err: Error) => void, attempt: number) => {
      ...
      if (
        ...
        (response.interactionModel.status === SKILL.INTERACTION_MODEL_STATUS.FAILURE && buildDetails !== undefined)
      ) {
        Logger.verbose(`Skill status response: ${JSON.stringify(response)}`);
      ...
      void vscode.window.withProgress(
        ...
        async () => {
          ...
          try {
            ...
            const skillPackageStatus = await getSkillPackageStatus(this.context, this.skillId, BRANCH_TO_STAGE[headBranch]);
