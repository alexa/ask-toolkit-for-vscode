...
export class AskStates extends AbstractResourcesConfig {
  ...
  constructor(skillFolderPath: string, content?: any) {
    const filePath = path.join(skillFolderPath, SKILL_FOLDER.HIDDEN_ASK_FOLDER, SKILL_FOLDER.ASK_STATES_JSON_CONFIG);
  ...
  setSkillMetaLastDeployHash(profile: string, lastDeployHash: string): void {
    this.setProperty(
      [ASK_STATES_JSON.PROFILES, profile, ASK_STATES_JSON.SKILL_METADATA.NAME, ASK_STATES_JSON.SKILL_METADATA.LAST_DEPLOY_HASH],
      lastDeployHash,
    );
  }

  getSkillMetaETag(profile: string): string {
    return this.getProperty([ASK_STATES_JSON.PROFILES, profile, ASK_STATES_JSON.SKILL_METADATA.NAME, ASK_STATES_JSON.SKILL_METADATA.ETAG]);
  }

  setSkillMetaETag(profile: string, eTag: string): void {
    this.setProperty([ASK_STATES_JSON.PROFILES, profile, ASK_STATES_JSON.SKILL_METADATA.NAME, ASK_STATES_JSON.SKILL_METADATA.ETAG], eTag);
  }
}
